...
// イベントがh1ve関連かどうかを判定
export function isH1veRelatedEvent(payload: WebhookPayload): boolean {
  // h1ve が作成したPRかチェック（ブランチ名で判定）
  ...
  // h1ve ラベルが付いているIssueかチェック
  if ("issue" in payload) {
    const issue = (payload as IssuePayload).issue;
    if (issue.labels.some((l) => l.name === "h1ve" || l.name === "auto-task")) {
      return true;
    }
  }
