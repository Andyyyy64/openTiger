...
import { tasks, events } from "@h1ve/db/schema";
...
async function handleIssueEvent(
  ...
  // 「h1ve」または「auto-task」ラベルが付いている場合、タスクを自動作成
  const shouldAutoTask = labels.some(
    (l) => l.name === "h1ve" || l.name === "auto-task"
  );
